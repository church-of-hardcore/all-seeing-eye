...
function AllSeeingEye:MainWindow()
	...
	local select_button = self.AceGUI:Create("Button")
	select_button:SetText("Select All")
	select_button:SetWidth(200)
	select_button:SetCallback("OnClick", function()
		editbox:HighlightText(0, -1)
		editbox:SetFocus()
	end)

	local ginvite_button = self.AceGUI:Create("Button")
	ginvite_button:SetText("Invite to Guild")
	ginvite_button:SetWidth(200)
	ginvite_button:SetCallback("OnClick", function()
		--self:AskWhoNext()
		self:OpenGuildInviter()
	end)

	window:AddChild(heading)
	window:AddChild(label)
	window:AddChild(editbox)
	window:AddChild(select_button)
	window:AddChild(ginvite_button)
end
